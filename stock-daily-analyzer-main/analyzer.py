...
from config import (
    ...
    DATA_PERIOD, SECTOR_STOCKS,
    MAX_RECOMMENDATIONS_PER_TYPE, MIN_VOLUME_RATIO,
    get_yfinance_ticker,
)
from market_data import fetch_a_share_universe, fetch_ohlc
from features import compute_history_today_diff, high_risk_divergence
...
class StockAnalyzer:
    ...
    def get_all_stocks(self) -> List[Tuple[str, str]]:
        """获取所有A股股票列表（除创业板外）"""
        try:
            stocks, source = fetch_a_share_universe()
            print(f"获取到 {len(stocks)} 只有效股票（来源 {source}，排除创业板和ST）")
            return stocks
        except Exception as e:
            print(f"获取股票列表失败，降级热门股: {e}")
    ...
    def fetch_stock_data(self, code: str, period: str = DATA_PERIOD) -> Optional[pd.DataFrame]:
        """获取单只股票数据（腾讯优先，Yahoo 兜底）"""
        return fetch_ohlc(code, period=period, min_bars=20)
    ...
    def analyze_single_stock(self, code: str, name: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 历史 vs 今日差异
            hist_today = compute_history_today_diff(data, lookback=30)
            ...
            return {
                ...
                'vol_ratio': vol_ratio,
                'ma_values': ma_values,
                'hist_today_diff': hist_today,
                'high_risk_divergence': high_risk_divergence(hist_today),
            }
        except Exception as e:
            return None
    ...
    def format_recommendation(self, stock: Dict, rec_type: str) -> Dict:
        ...
        # 置信度
        confidence = "中"
        if stock.get('high_risk_divergence'):
            confidence = "低"
            reasoning_parts.append("预警:历史与今日量价严重背离")
        elif stock.get('macd_golden_cross'):
            confidence = "高"
        elif stock['rsi'] < 30 or stock['rsi'] > 70:
            confidence = "高" if rec_type == '超卖反弹' else "低"

        diff = stock.get('hist_today_diff') or {}
        if diff.get('summary'):
            reasoning_parts.append(diff['summary'])
