...
from qf.data import CachedPriceProvider, DomesticFirstPriceProvider, YFinancePriceProvider
...
def _build_price_provider(cfg: ExperimentConfig):
    provider = (cfg.data.provider or "domestic").lower()
    if provider in {"yfinance", "yahoo"}:
        inner = YFinancePriceProvider()
    else:
        # default / "domestic" / "tencent" / "akshare"
        inner = DomesticFirstPriceProvider()
