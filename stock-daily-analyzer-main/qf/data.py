...
from market_data import fetch_ohlc, normalize_code
...
class DomesticFirstPriceProvider(PriceProvider):
    """Tencent hist first, yfinance fallback."""

    def history(self, code: str, *, start: str, end: str, timeout_seconds: int) -> Optional[PriceFrame]:
        df = fetch_ohlc(
            normalize_code(code),
            start=_ymd_to_compact(start),
            end=_ymd_to_compact(end),
