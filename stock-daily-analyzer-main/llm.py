...
def _gather_stock_context(stock: Dict) -> Dict:
    ...
    return {
        ...
        "hist_today_diff": stock.get("hist_today_diff") or {},
        "high_risk_divergence": bool(stock.get("high_risk_divergence")),
...
def _build_prompt(rec_type: str, contexts: List[Dict]) -> str:
    ...
    return (
        "你是资深交易助理。给定同一类型的候选股票，请综合以下四个维度进行深度分析，选择一只最值得买入的股票：\n"
        "1. **量化信号**：分析RSI、MACD、均线形态、量比等技术指标的有效性。\n"
        "2. **历史vs今日差异**：参考'hist_today_diff'字段，评估今日量价相对近30日是否异常背离；若 divergence_flags 含严重背离，需谨慎。\n"
        "3. **异动归因**：参考'history_attribution'字段，评估该股的历史股性、主力资金意图及上涨逻辑的持续性。\n"
        "4. **市场情绪**：结合新闻资讯，判断当前是否有明确的宏观或行业利好催化。\n\n"
